...
export const TaskViewSwitcher = ({ hideProjectFilter, forceProjectId }: TaskViewSwitcherProps) => {
    ...
    return (
        <Tabs defaultValue={view} onValueChange={setView} className="flex-1 w-full border-none rounded-lg">
            <TabsList className="w-full lg:w-auto p-0 pl-2 overflow-hidden">
                <TabsTrigger className="h-8 w-[calc(100%+12px)] lg:w-[calc(auto+12px])] bg-tertiary px-6" value="table">
                    Table
                </TabsTrigger>
                <TabsTrigger className="h-8 w-[calc(100%+12px)] lg:w-[calc(auto+12px])] bg-tertiary px-6" value="kanban">
                    Kanban
                </TabsTrigger>
            </TabsList>
            <div className="h-full flex flex-col overflow-auto bg-secondary rounded-xl p-4">
                ...
                {isLoadingTasks ? (
                    ...
                    <>
                        <TabsContent value="table" className="mt-0 border-t-0">
                            <DataTable columns={columns} data={tasks?.documents || []} />
                        </TabsContent>
                        <TabsContent value="kanban" className="mt-0 border-t-0">
